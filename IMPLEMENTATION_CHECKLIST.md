...
- [x] End-to-end smoke test (small synthetic shard, real `vit_small`-arch random-weight checkpoint via the CLI's `python -m fisseq_embeddings_pipeline.embed` entry point) produces `embeddings.parquet` with the expected shape -- the "(a) tiny random-init checkpoint" option SPEC.md §9.3 raises for CI, decided here since a real GPU/checkpoint isn't available in this environment anyway. **Additionally** re-run against the real `weights/cell_dino_vits8_pretrain_cp-37d20e9c.pth` checkpoint once it became available (CPU, `arch=vit_small patch_size=8 crop_size=128`): loads with 0 missing/0 unexpected keys and produces finite, non-degenerate `(4, 384)` embeddings end to end via the CLI — see `tests/unit/test_embed.py::test_load_cell_dino_and_embed_batch_against_real_checkpoint` (skipped when the checkpoint file isn't present, e.g. in CI, since it's gitignored). Which real checkpoint (this one, or the SPEC.md-documented `vit_large`/patch-16/bag-of-channels config) a real production run should actually point at remains an open product decision, not a code question — `EmbedCellsConfig`/`params.yaml` defaults are deliberately left unchanged.
