...
- [ ] `-resume` verified to actually skip unchanged per-batch stages on a second run — **not verified**: this sandbox has neither `nextflow`/`java` nor `docker` installed (checked: `which nextflow`/`java`/`docker` all empty), so no real `nextflow run` of any kind has been executed against this repo. Revisit wherever both are available.
...
- [ ] A full `nextflow run . --pipeline_dir <tiny-synthetic-experiment> -params-file params.yaml` completes with exit code 0 against the real (not stubbed) modules — **not run**, same sandbox limitation as Story 9.1's `-resume` bullet (no `nextflow`/`java`/`docker`). `tests/integration/test_integration.py` is written per SPEC.md §9.3's sketch (synthetic starcall-workflow-shaped tile fixture, a from-scratch random-init `vit_small` checkpoint on CPU for `EMBED_CELLS` per §9.3's option (a), a new `local` Nextflow profile in `nextflow.config` so it doesn't need `fisseq-embeddings-pipeline:latest` built) and skips cleanly (`pytest.skip`) when `nextflow` isn't on `PATH` — confirmed it collects and skips (5 tests) without error, but the pipeline itself has never actually been run through it. Needs a real run, and likely some threshold/param tuning, wherever `nextflow`+`java`+`docker` (or `-profile local` + a plain Python env) are available.
- [ ] Output directory tree matches SPEC.md §8 exactly (spot-check every file listed there exists) — asserted in `test_integration.py`'s `test_global_stage_outputs_exist`/etc., same unverified-execution caveat as above.
